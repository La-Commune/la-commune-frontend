...
export function SplashScreen() {
  ...
  return (
    <AnimatePresence>
      {visible && (
        <motion.div
          className="fixed inset-0 z-[9999] flex items-center justify-center bg-[#0c0b09] overflow-hidden"
          initial={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          transition={{ duration: 0.6, ease: "easeInOut" }}
          onClick={showSkip ? dismiss : undefined}
        >
          {/* Film grain — sutil textura analógica */}
          <div
          ...
          >
            ...
            <div className="w-6 h-px bg-[#c8956c] mx-auto mt-4" />
            <p className="font-mono text-[0.5rem] uppercase tracking-[0.4em] text-[#6b6458] mt-4">
              Hidalgo, MX
            </p>
          </motion.div>
          ...
          <AnimatePresence>
            {showSkip && (
              <motion.button
                ...
                onClick={dismiss}
                className="absolute bottom-12 font-mono text-[0.55rem] uppercase tracking-[0.3em] text-[#6b6458] hover:text-[#c8956c] transition-colors"
              >
                Continuar
              </motion.button>
            )}
